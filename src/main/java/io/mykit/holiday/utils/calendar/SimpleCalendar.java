...
public class SimpleCalendar extends BaseCalendar {
    ...
    public void calendar(Calendar cal) throws ParseException {
        ...
        if (m == 3) elements.get(tmp1).setColor("red"); //清明颜色
       ...
       // Pattern p = Pattern.compile("^(\\d{2})(\\d{2})([\\s\\*])(.+)$");
            ...
            if (matcher.matches()) {
                ...
                if (Integer.valueOf(matcher.group(1)).intValue() == (m + 1)) {
                    ...
                    if ("*".equals(tmp)){
                        ...
                        elements.get(Integer.valueOf(day) - 1).setColor("red");
                    }
                }
            }
        }
            ...
            if (matcher.matches()) {
                ...
                if (tmp1 >= 0 && tmp1 < n) {
                    ...
                    if (tmp2 >= 0 && tmp2 < this.length) {
                        ...
                        if (matcher.group(3).equals("*")) elements.get(tmp2).setColor("red");
                    }
                }
            }
        }
        ...
        if (m == 2 || m == 3) {
            ...
            if (m == estDay.getMonth())
                elements.get(estDay.getDate() - 1).setSolarFestival(elements.get(estDay.getDate() - 1).getSolarFestival() + " 复活节(Easter Sunday)");
        }
        //黑色星期五
        if ((this.firstWeek + 12) % 7 == 5)
            elements.get(12).setSolarFestival(elements.get(12).getSolarFestival() + "黑色星期五");
    }
}
